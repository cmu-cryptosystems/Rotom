...
def test_matmul_serialization():
    ...
    result = original_results[0] if isinstance(original_results, list) else original_results
    print(f"   Original result (first 8 elements): {result[:8] if hasattr(result, '__getitem__') else result}")
    ...
    # 9. Compare results
    print("\n9. Comparing results...")
    orig_result = original_results[0] if isinstance(original_results, list) else original_results
    reexec_result = reexec_results[0] if isinstance(reexec_results, list) else reexec_results
    ...
    if match:
        ...
        print(f"   Max difference: {np.max(np.abs(np.array(orig_result) - np.array(reexec_result)))}")
    ...
    print(f"   Execution order: {execution_order[:10]}..." if len(execution_order) > 10 else f"   Execution order: {execution_order}")
...
def test_multiple_operations():
    ...
    args = Namespace(n=4096, rolls=True, backend="toy", net="lan", 
                     cache=False, mock=False, serialize=False)
    ...
    result = original_results[0] if isinstance(original_results, list) else original_results
    print(f"   Created {len(file_paths)} files")
    print(f"   Original result: {result[:4] if hasattr(result, '__getitem__') else result}")
