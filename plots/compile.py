...
# MatMul - both sizes
data.append({'Benchmark': 'MatMul', 'n': 8192, 'Rotom': 0.10, 'Fhelipe': 2.19, 'Viaduct-HE-e1-o0': 72.02, 'Viaduct-HE-e2-o1': 17603.89})
data.append({'Benchmark': 'MatMul', 'n': 32768, 'Rotom': 0.11, 'Fhelipe': 6.89, 'Viaduct-HE-e1-o0': 997.45, 'Viaduct-HE-e2-o1': np.nan})

# Double-MatMul - both sizes
data.append({'Benchmark': 'Double-MatMul', 'n': 8192, 'Rotom': 0.28, 'Fhelipe': 2.73, 'Viaduct-HE-e1-o0': 6346.91, 'Viaduct-HE-e2-o1': 50040.36})
data.append({'Benchmark': 'Double-MatMul', 'n': 32768, 'Rotom': 0.32, 'Fhelipe': 11.05, 'Viaduct-HE-e1-o0': np.nan, 'Viaduct-HE-e2-o1': np.nan})

# TTM - both sizes
data.append({'Benchmark': 'TTM', 'n': 8192, 'Rotom': 1.58, 'Fhelipe': 14.91, 'Viaduct-HE-e1-o0': np.nan, 'Viaduct-HE-e2-o1': np.nan})
data.append({'Benchmark': 'TTM', 'n': 32768, 'Rotom': 1.34, 'Fhelipe': 4.84, 'Viaduct-HE-e1-o0': np.nan, 'Viaduct-HE-e2-o1': np.nan})

# Convolution - both sizes
data.append({'Benchmark': 'Convolution', 'n': 8192, 'Rotom': 1.64, 'Fhelipe': 1.67, 'Viaduct-HE-e1-o0': 81.60, 'Viaduct-HE-e2-o1': 1259.03})
data.append({'Benchmark': 'Convolution', 'n': 32768, 'Rotom': 21.97, 'Fhelipe': 15.89, 'Viaduct-HE-e1-o0': 1579.479, 'Viaduct-HE-e2-o1': 22748.81})

# LogReg-MatVecMul - both sizes
data.append({'Benchmark': 'LogReg-MatVecMul', 'n': 8192, 'Rotom': 3.00, 'Fhelipe': 2.34, 'Viaduct-HE-e1-o0': 58.76, 'Viaduct-HE-e2-o1': 1052.58})
data.append({'Benchmark': 'LogReg-MatVecMul', 'n': 32768, 'Rotom': 1.67, 'Fhelipe': 2.40, 'Viaduct-HE-e1-o0': 58.455, 'Viaduct-HE-e2-o1': 535.105})

# Bert Attention - both sizes
data.append({'Benchmark': 'Bert Attention', 'n': 8192, 'Rotom': 243.56, 'Fhelipe': 113.63, 'Viaduct-HE-e1-o0': np.nan, 'Viaduct-HE-e2-o1': np.nan})
data.append({'Benchmark': 'Bert Attention', 'n': 32768, 'Rotom': 5.42, 'Fhelipe': 49.47, 'Viaduct-HE-e1-o0': np.nan, 'Viaduct-HE-e2-o1': np.nan})
    ...
    for pos, size in enumerate(sizes):
        ...
        for k, system in enumerate(systems):
            if not size_data.empty:
                ...
                orig_value = orig_size_data[system].iloc[0] if not orig_size_data.empty else np.nan
                
                # Draw the bar (always draw a bar, even for timeouts)
                bar = ax.bar(pos + (k - 1.5) * bar_width, value, bar_width, 
                      color=colors[k], alpha=0.8, label=system if i == 0 and pos == 0 else "")
                
                # If this is a timeout, mark it appropriately
                if np.isnan(orig_value):
                    # Add timeout indicator at a visible height
                    ax.text(pos + (k - 1.5) * bar_width, 0.1, "t-out", 
...
fig.legend(handles, labels, loc='lower center', ncol=5, fontsize=18, bbox_to_anchor=(0.5, 0.05))
