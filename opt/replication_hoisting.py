    ...
    for kernel in candidate.post_order():
        ...
        match kernel.op:
            case KernelOp.REPLICATE:
                if kernel.cs[0].op == KernelOp.CS and kernel.cs[0].layout.term.op == TensorOp.TENSOR:
                    # get cs kernel 
                    cs_kernel = kernel.cs[0]

                    # create a new CS kernel with replicated slot dims 
                    replicated_slot_dims = kernel.layout.slot_dims
                    new_cs_layout = Layout(cs_kernel.layout.term, cs_kernel.layout.rolls, copy(replicated_slot_dims), cs_kernel.layout.offset, cs_kernel.layout.n, cs_kernel.layout.secret)
                    new_cs_kernel = Kernel(KernelOp.CS, [cs_kernel.cs[0]], new_cs_layout)
                    ...
                    if new_cs_layout.get_dims() != kernel.layout.get_dims():
                        # receate replication 
                        replicated_kernel = Kernel(KernelOp.REPLICATE, [new_cs_kernel], copy(kernel.layout))
