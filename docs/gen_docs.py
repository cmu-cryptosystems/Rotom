...
def clean_docs(output_dir="."):
    ...
    if doctrees_found:
        ...
        for doctrees_path in doctrees_progress:
            doctrees_progress.set_postfix(dir=os.path.basename(os.path.dirname(doctrees_path)))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate Sphinx documentation for Rotom")
    parser.add_argument("--output-dir", default=".", help="Output directory for documentation (default: current directory)")
    parser.add_argument("--format", default="html", help="Output format: html, pdf, or both (default: html)")
    parser.add_argument("--clean", action="store_true", help="Clean output directory before generating")
    parser.add_argument("--clean-only", action="store_true", help="Only clean, don't generate documentation")
