...
Rescale operations divide tensors by a power of 2, which is commonly used in
homomorphic encryption to manage scale factors.

Key functions:
- gen_rescale: Main function for generating rescale operation
"""
...
def gen_rescale(term, kernels):
    """Generates kernels for rescale operations.

    This function creates kernels for rescale operations that divide
    tensors by 2^scale_exp. The rescale operation maintains the same layout
    as the input tensor.

    Args:
        term: TensorTerm representing the rescale operation
        kernels: List of input kernels to generate rescale kernels for

    Returns:
        Set of Kernel objects representing rescale operation kernels
    """
    output_kernels = set()
