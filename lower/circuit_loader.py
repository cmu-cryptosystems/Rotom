    ...
    def parse_instruction(instr_line: str) -> Tuple[int, bool, str, List]:
        ...
        if op_str.startswith("pack"):
            ...
            if len(op_parts) >= 2:
                ...
                operands = [
                    int(x) if x.lstrip("-").isdigit() else x for x in op_parts[1:]
                ]

                # Map symbols to operations
                op_map = {"+": "add", "-": "sub", "*": "mul", "<<": "rot"}
...
class CircuitLoader:
    ...
    def load(self) -> Dict[str, Any]:
        ...
        manifest_path = os.path.join(
            self.circuit_dir, f"{self.circuit_name}_manifest.json"
        )
        ...
        for kernel_meta in self.manifest["kernels"]:
            ...
            kernel_data.append({"metadata": kernel_meta, "instructions": kernel_instrs})
