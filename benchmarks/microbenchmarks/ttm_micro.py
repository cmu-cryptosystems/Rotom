...
def ttm_micro(n):
    ...
    tensor_layout = Layout(
        term, [], [Dim.parse(f"[1:32:2]"), Dim.parse(f"[0:64:1]"), Dim.parse(f"[1:2:1]"), Dim.parse(f"[2:64:1]")], {}, n, True
    )
    ...
    tensor_layout = Layout(
        term, [], [Dim.parse(f"[1:64:1]"), Dim.parse(f"[2]"), Dim.parse(f"[0:64:1]")], {}, n, True
    )
    ...
    replicated_t_layout = Layout(
        term, [], [Dim.parse(f"[64]"), Dim.parse(f"[1:32:2]"), Dim.parse(f"[0:64:1]"), Dim.parse(f"[1:2:1]"), Dim.parse(f"[2:64:1]")], {}, n, True
    )
    t = Kernel(KernelOp.REPLICATE, [t], layout=replicated_t_layout)

    replicated_m_layout = Layout(
        term, [], [Dim.parse(f"[64]"), Dim.parse(f"[32]"), Dim.parse(f"[1:64:1]"), Dim.parse(f"[2]"), Dim.parse(f"[0:64:1]")], {}, n, True
    )
    m = Kernel(KernelOp.REPLICATE, [m], layout=replicated_m_layout)

    # roll m 
    rolled_m_layout = Layout(
        term, [Roll(Dim.parse(f"[1:64:1]"), Dim.parse(f"[64]"))], [Dim.parse(f"[1:64:1]"), Dim.parse(f"[32]"), Dim.parse(f"[64]"), Dim.parse(f"[2]"), Dim.parse(f"[0:64:1]")], {}, n, True
    )
    ...
    matmul_layout = Layout(
        term, [Roll(Dim.parse(f"[2:64:1]"), Dim.parse(f"[0:64:1]"))], [Dim.parse(f"[2:64:1]"), Dim.parse(f"[1:32:2]"), Dim.parse(f"[0:64:1]"), Dim.parse(f"[1:2:1]"), Dim.parse(f"[G:64]")], {}, n, True
    )
