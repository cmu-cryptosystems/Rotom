...
def convolution_32768():
    ...
    inputs["a"] = np.array(
        [
            [[np.random.choice(range(2)) for i in range(dim_size)] for j in range(dim_size)]
            for _ in range(input_channels)
        ]
    )
    inputs["b"] = np.array([[[[np.random.choice(range(2)) for i in range(f_size)] for j in range(f_size)]]])
